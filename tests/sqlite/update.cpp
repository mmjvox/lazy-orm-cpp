...
TEST_CASE( "Factorials are computed", "[Lazy_UPDATE]" ) {
    REQUIRE( Catch::trim(update1()) == R"(UPDATE student SET `age`='6',`hair`='pink',`name`='anya' WHERE (`grade` in (1,5,7,9)) GROUP BY `group1`,`group2` ORDER BY `num1`,`num2` LIMIT 5 OFFSET 6 ;)" );
}
